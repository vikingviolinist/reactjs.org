...
In this example, there are two components. There is a parent `Accordion` component that renders two separate `Panel`s like so:
...
You will give control of the `Panel`'s `isActive` to its parent component. This means that the parent component will pass `isActive` to `Panel` as a prop. Start by **removing this line** from the `Panel` component:
